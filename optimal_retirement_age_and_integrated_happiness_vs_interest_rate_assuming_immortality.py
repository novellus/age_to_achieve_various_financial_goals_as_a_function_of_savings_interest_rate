...
if __name__ == '__main__':
    ...
    descriptor = (f'Maximim average happiness over lifetime and optimal retirement age as a function of interest rate. (evaluated at discrete 1-year intervals)\n' +
                  f'initial_age = {initial_age}, maximum_retirement_age = {maximum_retirement_age+1}, inflation_rate = {(inflation_rate-1)*100:.3}% (annual), initial_money = {initial_money}, annual_gross_earn_rate = {annual_gross_earn_rate}, annual_cost_of_living = {annual_cost_of_living}\n' +
    ...
    plt.xlim(1.0, 1.1)
    plt.ylim(initial_age, maximum_retirement_age + 4)
    ...
    # # reference indicating enopugh capital accrued to surive to maximum_retirement_age
    # region_patches = []
    # previous_y = None
    # for i_region, difference in enumerate(np.linspace(0, 80, 5)):
    #     x, y = [initial_age, maximum_retirement_age], [maximum_retirement_age - initial_age - difference, 0 - difference]
    ...
    #     region_patch = matplotlib.patches.Patch(color=colors[(i_region)%len(colors)], label=f'enough savings to survive until age {int(maximum_retirement_age - difference)}')
    ...
    #     plt.plot([initial_age, maximum_retirement_age], [8, 8 + slope * (maximum_retirement_age - initial_age)], c=colors[(i_plot+1)%len(colors)], marker=None, linestyle='--', label=f'reference slope {slope:.3}')
    ...
    for interest_rate in interest_rates:
        ...
        for retirement_age in range(initial_age, maximum_retirement_age + 1):
            ...
            if working_happiness >= free_happiness:
                ...
                data_run_meta['integrated_happiness'].append(float('inf'))
                data_run_meta['death_age'].append(float('inf'))
            else:
