...
if __name__ == '__main__':
    ...
    descriptor = (f'Years of survival after retirement as a function of age of retirement and working happiness. (evaluated at 1-year intervals)\n' + 
                  ...
                  f'working_happiness = {working_happiness}, free_happiness = {free_happiness}')
    ...
    # plt.xlim(0.0, 10.0)
    plt.ylim(initial_age, likely_death_age + 1)
    ...
    interest_rates = list(np.linspace(1, 1.04, 1000))
    ...
    plt.gca().set_yticks(np.linspace(*plt.gca().get_ybound(), 7))
    plt.legend(loc=2)

    plt.twinx()
    plt.ylabel('maximum happiness')
    ...
    plt.ylim(math.floor(min(working_happiness, free_happiness)*10-1)/10, math.ceil(max(working_happiness, free_happiness)*10+1)/10)
    plt.gca().set_yticks(np.linspace(*plt.gca().get_ybound(), 7))
    plt.legend(loc=1)
